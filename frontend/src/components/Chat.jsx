...
const Chat = () => {
  ...
  useEffect(() => {
    if (!lastTrackedOrderId) {
      // No order to track, disconnect if connected
    ...
    const connectWebSocket = () => {
      try {
        const ws = new WebSocket('ws://localhost:8004/ws/fulfillment');
        
        ws.onopen = () => {
          console.log('✅ WebSocket connected for order:', lastTrackedOrderId);
          setWsConnected(true);
          // Send initial message to keep connection alive
          ws.send(JSON.stringify({ type: 'subscribe', order_id: lastTrackedOrderId }));
        };
        ...
        ws.onmessage = async (event) => {
          try {
            ...
            if (data.type === 'delivery_update' && data.order_id === lastTrackedOrderId) {
              const fulfillment = data.fulfillment;
              const status = fulfillment.current_status;

              // Send automatic notification based on status
              if (status === 'OUT_FOR_DELIVERY') {
                const deliveryWindow = fulfillment.delivery_window || 'your selected slot';
                const windowTime = deliveryWindow === 'morning' ? '6AM-12PM' : 
                                   deliveryWindow === 'afternoon' ? '12PM-6PM' : 
                                   deliveryWindow === 'evening' ? '6PM-10PM' : 'your time slot';
                
                const otp = fulfillment.delivery_otp || 'N/A';
                const deliveryPerson = fulfillment.delivery_boy_name || 'delivery partner';
                const deliveryPhone = fulfillment.delivery_boy_phone || '';

                const message = `🚚 Great news! Your order is out for delivery!\n\n` +
                  `📦 Delivery Window: ${windowTime}\n` +
                  `👤 Delivery Person: ${deliveryPerson}\n` +
                  `📞 Contact: ${deliveryPhone}\n` +
                  `🔐 OTP: ${otp}\n\n` +
                  `Please keep your OTP ready for verification!`;

                await appendAgentMessage(message);
              } else if (status === 'DELIVERED') {
                const message = `✅ Your order has been delivered! We hope you love your purchase!\n\n` +
                  `How was your experience? I can help you with:\n` +
                  `• Share Feedback 💬\n` +
                  `• Start a Return 📦\n` +
                  `• Exchange Size 🔁`;
