...
def save_session_to_supabase(session_token: str, session: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        if response.status_code in [200, 201]:
            logger.info(f"[SUPABASE] ✅ Session saved: customer_id={customer_id_str}, phone={phone_str}, token={session_token[:15]}...")
            return True
        else:
            logger.error(f"[SUPABASE] ❌ SAVE FAILED: Status={response.status_code}, Body={response.text}, Record={session_record}")
            return False
